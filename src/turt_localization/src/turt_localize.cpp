...
class MyNode : public rclcpp::Node
{
  public:
      MyNode() : Node("my_node"),
                 ...
                 radius_r(0.03302), radius_l(0.03302), width(0.287), first_reading(true) // put x, y, theta, last left/right pos,
      {
  ...
  private:
    ...
    // constant value of radius_r for calculation
    const double radius_r, radius_l, width; 
    ...
    void my_callback(sensor_msgs::msg::JointState msg)
    {
      ...
      double deltaD{(radius_l * delta_q_l + radius_r * delta_q_r) / 2.0},
          delta_theta{(radius_l * delta_q_l - radius_r * delta_q_r) / width};
      ...
      out_msg.child_frame_id = "waffle2/base_footprint_fhj"; // to the base frame
      ...
      transform.child_frame_id = "waffle2/base_footprint_fhj";

      my_publisher->publish(out_msg);
      tf_broadcaster->sendTransform(transform);

      RCLCPP_INFO(this->get_logger(), "Publishing odom_frame_fhj");
